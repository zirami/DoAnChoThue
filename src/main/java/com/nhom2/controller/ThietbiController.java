...
public class ThietbiController {
	...
	public String themLoai(ModelMap model, @ModelAttribute("loaithietbi_moi") LOAITHIETBI ltb) {
		model.addAttribute("insert", new LoaiThietBiDAO().save(factory, ltb));
		return home(model);
	}
	
	// UPDATE LOẠI THIẾT BI GET
	@RequestMapping(value = "loaiTB/edit/{id}", method = RequestMethod.GET)
	public String show_form_edit_loaiTB(ModelMap model, @ModelAttribute("loaithietbi_sua") LOAITHIETBI loaithietbi_sua, @PathVariable Integer id) {
	...
	// UPDATE LOẠI THIẾT BI POST
		...
		public RedirectView updateLoaiTB(@Valid @ModelAttribute("loaithietbi_sua") LOAITHIETBI loaithietbi_sua,  BindingResult reusult, RedirectAttributes model) {
			System.out.println("has error: " + reusult.getFieldErrors().toString());
			model.addFlashAttribute("sua_saidinhdang", reusult.hasErrors());
			...
			model.addFlashAttribute("update", new LoaiThietBiDAO().update(factory, loaithietbi_sua)); // Xử lý thông báo thành công
